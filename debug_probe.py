...
SCHEMA_PATH  = Path("configs/json_output_schema.json")
...
def probe_db():
    ...
    if not DB_PATH.exists():
        report.record("db", "DB file exists", "fail", f"{DB_PATH} not found")
        return
    ...
    try:
        ...
        if mode == "wal":
            report.record("db", "WAL mode active", "pass")
        else:
            report.record("db", "WAL mode active", "warn",
        ...
        if "defects" in tables:
            ...
            report.record("db", "Defects row count", "pass", f"{n} rows")
        ...
        if violations:
            report.record("db", "Foreign key violations", "fail",
                          f"{len(violations)} violations: {[dict(v) for v in violations[:3]]}")
        else:
            report.record("db", "Foreign key violations", "pass")

        # 1g. growth_acceleration NULL check
        if "defects" in tables:
            nulls = conn.execute(
                "SELECT COUNT(*) FROM defects WHERE growth_acceleration IS NULL"
            ).fetchone()[0]
            if nulls > 0:
                report.record("db", "growth_acceleration NULL rows", "fail",
                              f"{nulls} rows have NULL — [RC-09] not applied")
            else:
                report.record("db", "growth_acceleration NULL rows", "pass")

        # 1h. warnings_json parseable
        if "inspections" in tables:
            bad_json = 0
            for row in conn.execute("SELECT id, warnings_json FROM inspections").fetchall():
                try:
                    json.loads(row["warnings_json"])
                except json.JSONDecodeError:
                    bad_json += 1
            if bad_json:
                report.record("db", "warnings_json parseable", "fail",
                              f"{bad_json} rows have malformed JSON — [RC-12] not applied")
            else:
                report.record("db", "warnings_json parseable", "pass")
...
def probe_api():
    ...
    try:
        ...
        with urlopen(req, timeout=5) as resp:
            ...
            if cors:
                report.record("api", "CORS header present", "pass", f"Allow-Origin: {cors}")
            else:
                report.record("api", "CORS header present", "fail",
                              "[RC-17] CORS missing — frontend JS fetch will be blocked")
    except Exception:
        report.record("api", "CORS header check", "warn", "Could not verify — server may be down")
...
def probe_tiles():
    ...
    if not FRONTEND_DIR.exists():
        report.record("tiles", "frontend/ directory", "fail", f"{FRONTEND_DIR} not found")
        return
    report.record("tiles", "frontend/ directory", "pass")

    if not tile_dir.exists():
        report.record("tiles", "tiles/ directory", "fail",
                      "[RC-15] Tile directory missing — facade map will be blank")
        return
    ...
    for zoom in range(3):
        ...
        if not zoom_dir.exists():
            report.record("tiles", f"Zoom level {zoom}", "fail",
                          f"{zoom_dir} missing — [RC-15] map will fail at zoom {zoom}")
            continue
        tiles = list(zoom_dir.rglob("*.png")) + list(zoom_dir.rglob("*.jpg"))
        if tiles:
            report.record("tiles", f"Zoom level {zoom}", "pass", f"{len(tiles)} tiles")
        else:
            report.record("tiles", f"Zoom level {zoom}", "fail", f"No tile files in {zoom_dir}")
    ...
    for jsfname in ["components/map.js", "components/charts.js", "components/sidebar.js"]:
        p = FRONTEND_DIR / jsfname
        status = "pass" if p.exists() else "warn"
        detail = "" if p.exists() else f"{p} not found — component will be missing"
        report.record("tiles", f"Component: {jsfname}", status, detail)
...
def probe_pipeline():
    ...
    # Module imports
    try:
        from pipeline import CortexPipeline, PIPELINE_VERSION
    ...
    if not config_path.exists():
        report.record("pipeline", "configs/config.json exists", "fail")
    else:
        try:
            with open(config_path) as f:
                cfg = json.load(f)
            required_keys = ["gsd_mm_per_px", "temporal_tolerance_px", "elapsed_months"]
            missing = [k for k in required_keys if k not in cfg]
            if missing:
                report.record("pipeline", "config.json keys", "warn",
                              f"Missing optional keys: {missing}")
            else:
                report.record("pipeline", "config.json keys", "pass")
        except json.JSONDecodeError as exc:
            report.record("pipeline", "config.json parseable", "fail", str(exc))

    # SIFT cache (hash-based)
    try:
        from pipeline import SIFTCache
        cache = SIFTCache()
        # Verify _hash method works (no image needed)
        dummy_path = Path("probe_dummy.jpg")
        dummy_path.write_bytes(b"\xff\xd8\xff\xe0" + b"\x00" * 100)  # minimal JPEG header
        try:
            h = cache._hash(str(dummy_path))
            assert len(h) == 64, "SHA-256 should be 64 hex chars"
            report.record("pipeline", "SIFT cache SHA-256 keying", "pass", f"hash={h[:16]}...")
        finally:
            dummy_path.unlink(missing_ok=True)
    except Exception as exc:
        report.record("pipeline", "SIFT cache SHA-256 keying", "fail", str(exc))
    ...
    try:
        ...
        if MAX_WORKERS <= expected_max:
            report.record("pipeline", "ThreadPoolExecutor worker cap", "pass",
                          f"MAX_WORKERS={MAX_WORKERS}, CPUs={cpu}")
        else:
            report.record("pipeline", "ThreadPoolExecutor worker cap", "fail",
                          f"MAX_WORKERS={MAX_WORKERS} > CPU-1={expected_max} — [RC-01] not applied")
    except ImportError:
        report.record("pipeline", "MAX_WORKERS constant", "warn", "Not found in pipeline.py")

    # pipeline_warnings in output schema
    try:
        from pipeline import InspectionResult
        import dataclasses
        fields = {f.name for f in dataclasses.fields(InspectionResult)}
        if "pipeline_warnings" in fields:
            report.record("pipeline", "pipeline_warnings field", "pass")
        else:
            report.record("pipeline", "pipeline_warnings field", "fail",
                          "[RC-06] InspectionResult missing pipeline_warnings — frontend banner broken")
        if "pipeline_version" in fields:
            report.record("pipeline", "pipeline_version field", "pass")
        else:
            report.record("pipeline", "pipeline_version field", "fail",
                          "[RC-05] pipeline_version missing — audit trail broken")
    except Exception as exc:
        report.record("pipeline", "InspectionResult fields", "warn", str(exc))
...
def probe_schema():
    ...
    if not SCHEMA_PATH.exists():
        report.record("schema", "json_output_schema.json exists", "fail",
    ...
    # vi_class enum check — must be lowercase  ([RC schema mismatch fix])
    try:
        ...
        if actual == expected:
            ...
            report.record("schema", "vi_class enum lowercase", "fail",
                          f"Mismatch: {diff} — schema/pipeline diverged")
    except KeyError:
